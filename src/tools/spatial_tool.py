...
try:
    ...
    OMNIGUARD_PATH = _r / "external" / "OmniGuard-main" if (_r / "external" / "OmniGuard-main").exists() else _r / "OmniGuard-main"
    MESORCH_PATH = _r / "external" / "Mesorch-main" if (_r / "external" / "Mesorch-main").exists() else _r / "Mesorch-main"

_r2 = Path(__file__).resolve().parents[2]
TRUFOR_ROOT = (
    _r2 / "external" / "TruFor-main" / "TruFor_train_test"
    if (_r2 / "external" / "TruFor-main").exists()
    else _r2 / "TruFor-main" / "TruFor_train_test"
)
...
class SpatialAnalysisTool(BaseTool):
    ...
    def __init__(
        ...
        if use_mvss_mask is not None:
            ...
            _mvss_base = Path(__file__).resolve().parents[2]
            _mvss_default = (
                _mvss_base / "external" / "MVSS-Net-master" / "ckpt" / "mvssnet_casia.pt"
                if (_mvss_base / "external" / "MVSS-Net-master").exists()
                else _mvss_base / "MVSS-Net-master" / "ckpt" / "mvssnet_casia.pt"
            )
            mvss_ckpt = Path(os.environ.get("MAIFS_MVSS_CHECKPOINT", _mvss_default))
