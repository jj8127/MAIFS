...
CPU_PYTHON="${CPU_PYTHON:-${REPO_ROOT}/.venv-rpi5-cpu/bin/python}"
...
exec "${HOST_PYTHON}" "${COMMON_BENCH}" \
  --mode cpu \
  --image "${IMAGE}" \
  --runner-python "${CPU_PYTHON}" \
  --output-json "${ENV_DIR}/results/${STAMP}_rpi5_cpu_latency.json" \
  --output-log "${ENV_DIR}/logs/${STAMP}_rpi5_cpu_latency.log" \
  "$@"
