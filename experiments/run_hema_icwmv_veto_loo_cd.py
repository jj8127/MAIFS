...
WEIGHTING_MODE = "inverse_confidence"
WEIGHTING_ALPHA = 1.0
...
def effective_weighting_alpha() -> float:
    if WEIGHTING_MODE == "equal_weight":
        return 0.0
    return max(float(WEIGHTING_ALPHA), 0.0)


def weighting_tag() -> str:
    return f"alpha{effective_weighting_alpha():.1f}".replace(".", "p")


def weighting_pair(m: Dict, s: Dict) -> Tuple[float, float]:
    alpha = effective_weighting_alpha()
    if alpha <= 0.0:
        return 1.0, 1.0
...
def main() -> None:
    args = parse_args()
    global WEIGHTING_MODE, WEIGHTING_ALPHA
    ...
    final_results = {
        "config": {
            ...
            "weighting_mode": args.weighting_mode,
            "weighting_alpha": float(args.weighting_alpha),
            "effective_weighting_alpha": effective_weighting_alpha(),
        },
        "results": {},
    }
    ...
    out_path = out_dir / f"hema_icwmv_veto_loo_cd_{weighting_tag()}_{ts}.json"
