...
4. Raspberry Pi 5의 CPU, USB Edge TPU, PCIe 연결 HAT에서 1단계 기준 구조를 직접 측정하고, 동시에 ARV 전체 구조의 배포 평가 범위를 정직하게 구분한다.
...
추론 단계에서는 기본 분류기와 보조 모델의 출력을 직접 평균하지 않고, 각 모델의 최대 확률에 역비례하는 가중치를 둔다. 목적은 어느 한 모델의 과도한 확신이 결과를 완전히 지배하지 않도록 하는 데 있다.

식 (3), 식 (4)는 기본 분류기와 보조 모델의 가중치이고, 식 (5)부터 식 (7)까지는 최종 결합 점수이다.

식 (3)  
$$w_{\text{base}} = \frac{1}{\max(P_{\text{base}})}$$

식 (4)  
$$w_{\text{aux}} = \frac{1}{\max(P_{\text{aux}})}$$
...
`ai_generated` 클래스는 보조 모델이 다루지 않으므로 기본 분류기만이 담당한다. 1단계의 역할은 가능한 한 많은 유익한 교정 후보를 만드는 것이다. 반대로, 해로운 변경을 억제하는 역할은 2단계 ARV가 맡는다.
...
온디바이스 실측은 현재 1단계 기준 구조에 대해서만 확보되어 있다. 즉, 기본 분류기와 보조 모델, 그리고 두 모델의 고정 결합 규칙까지는 Raspberry Pi 5에서 측정되었지만, ARV가 포함된 2단계 전체 파이프라인의 종단간 지연은 아직 직접 측정하지 않았다. 따라서 본 절은 현재 확보된 배포 결과를 정리하고, 동시에 본 논문이 직접 주장할 수 있는 범위를 분명히 하는 데 목적이 있다.
...
따라서 본 논문에서 직접 주장할 수 있는 온디바이스 결과는 1단계 기준 구조의 Raspberry Pi 5 CPU, USB Edge TPU, PCIe 연결 HAT 실측값에 한정된다. ARV는 구조적으로 경량 후처리 모듈이지만, 아직 2단계 전체 파이프라인의 Raspberry Pi 5 종단간 측정은 직접 수행하지 않았다. 따라서 본 논문은 1단계 기준 구조의 배포 가능성은 실측으로 제시하되, ARV 전체 구조의 배포 성능은 향후 과제로 남겨 둔다.
...
마지막으로 온디바이스 관점에서 ARV는 단계 수를 늘리지만, 추가되는 계산은 대형 신경망이 아니라 경량 메타 분류기다. 본 연구는 1단계 기준 구조에 대해 CPU, USB Edge TPU, PCIe 연결 HAT의 실측값을 확보했지만, 아직 ARV 전체 파이프라인의 Raspberry Pi 5 종단간 측정은 수행하지 않았다. 이는 ARV를 메인 주제로 다루더라도, 현재 확보된 실험 범위 안에서만 주장하기 위한 의도적 제한이다.
...
실험 결과, ARV를 포함한 최종 시스템은 기본 분류기 단독의 avg macro-F1 0.9581을 0.9652로 향상시켰고, 1단계 결합에서 발생하던 역교정 건수를 40건에서 18건으로 줄였다. pooled sample-level 비교에서도 macro-F1은 0.9562에서 0.9643으로 상승했고, 2000회 paired bootstrap 95% 신뢰구간은 `[0.0045, 0.0117]`, exact McNemar test는 `p=5.8×10^-5`로 유의하였다. 또한 단순 softmax 평균 앙상블과 logistic stacking 같은 외부 baseline보다도 ARV가 더 높은 평균 F1과 순이득을 보였다. 특히 OpenSDI에서의 F1 개선은 ARV가 학습 분포와 차이가 큰 환경에서의 해로운 판정 변경을 효과적으로 제어함을 보여준다. 추가 서버 실험과 10-seed 반복 검증에서는 미세조정된 MobileCLIP 기반 기본 분류기에서도 ARV가 가장 높은 F1을 보였고, strong backbone 두 종류를 합친 경우 역교정 총합은 평균 30.2건으로 줄어 평균 63.6% 감소하였다. 반면 aigenproxy에서는 유익한 변경까지 함께 되돌리는 과보수성이 남아 있다. 현재 Raspberry Pi 5에서의 온디바이스 실측은 1단계 기준 구조까지 확보되어 있으며, 평균 총 지연은 CPU 123.72ms, USB Edge TPU 64.56ms, PCIe 연결 HAT 54.17ms였다. 반면 ARV를 포함한 2단계 전체 구조의 종단간 측정은 향후 과제로 남아 있다.

향후 연구에서는 첫째, Raspberry Pi 5에서 ARV 전체 파이프라인의 종단간 지연을 직접 측정하고, 둘째, CPU, USB Edge TPU, PCIe 연결 HAT 환경에서 ARV 추가 비용을 정량적으로 비교하며, 셋째, 다양한 기본 분류기에서 ARV의 유효 범위를 더 체계적으로 분석할 계획이다.
